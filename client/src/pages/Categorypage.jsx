...
const Categorypage = () => {
  ...
  const highlightMatch = (text, highlight) => {
    ...
    return parts.map((part, i) =>
      part.toLowerCase() === highlight.toLowerCase() ? (
        <mark key={i} className="bg-amber-100 text-amber-800 rounded">{part}</mark>
      ) : part
    )
  }
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white border border-gray-200 rounded-2xl p-4 mb-4 shadow-sm">
          ...
          <div className="flex flex-col sm:flex-row gap-3 mt-3 sm:mt-0 w-full sm:w-auto">
            ...
            <button
              onClick={() => setopenUploadCategory(true)}
              className="text-sm font-semibold bg-gradient-to-r from-amber-500 to-orange-500 text-white px-5 py-2 rounded-xl shadow hover:from-amber-600 hover:to-orange-600 transition"
            >
              + Add Category
            </button>
          </div>
        ...
        <div className="text-sm text-gray-600 mb-2">
          Total: {categoryData.length} | Showing: {filteredData.length}
        </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 place-items-center gap-5 py-4">
            {loading
              ? Array.from({ length: itemsPerPage }).map((_, i) => (
                  <div
                    key={i}
                    className="bg-gray-100 rounded-2xl shadow-sm p-4 w-40 animate-pulse"
                  >
                    <div className="w-28 h-28 bg-gray-200 rounded-xl mx-auto"></div>
                    <div className="h-3 bg-gray-300 mt-3 rounded w-3/4 mx-auto"></div>
                  </div>
                ))
              : paginatedData.map((category) => (
                  <div
                    key={category._id}
                    className="bg-white border border-gray-200 rounded-2xl shadow-sm p-4 flex flex-col items-center w-40 hover:shadow-md transition-transform hover:scale-[1.03]"
                  >
                    <div className="w-28 h-28 flex justify-center items-center overflow-hidden rounded-xl bg-gray-50">
                      <img
                        src={category.image}
                        alt={category.name}
                        className="w-full h-full object-contain transition-transform duration-200 hover:scale-105"
                    ...
                    <p className="mt-2 text-sm font-semibold text-gray-800 text-center line-clamp-1">
                      {highlightMatch(category.name, searchTerm)}
                    </p>
                    <div className="flex w-full gap-2 mt-2">
                      <button
                        ...
                        className="flex-1 text-sm font-medium bg-green-100 text-green-700 py-1 rounded-md hover:bg-green-200 transition"
                      >
                        Edit
                      </button>
                      <button
                        ...
                        className="flex-1 text-sm font-medium bg-red-100 text-red-600 py-1 rounded-md hover:bg-red-200 transition"
                      >
                        Delete
                      </button>
                    </div>
