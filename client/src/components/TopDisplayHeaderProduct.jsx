...
import { priceWithDiscount } from "../utils/helpers/PriceWithDiscount";
import { validURLConvert } from "../utils/helpers/validURLConvert";
import OutOfStockOverlay from "./OutOfStockOverlay";
      ...
      <Link to={url} className="block">
        <div className="relative h-32 bg-white flex items-center justify-center p-2">
          ...
          <img
            ...
            loading="lazy"
          />
          {item.stock <= 0 && <OutOfStockOverlay variant="default" showSoldOut={true} />}
        </div>
...
const TopDisplayHeaderProduct = () => {
  ...
  const checkScroll = useCallback(throttle(() => {
    ...
    const scrollPosition = scrollLeft / (scrollWidth - clientWidth);
    const indicatorIndex = Math.round(scrollPosition * 2); // 3 positions
    setCurrentScrollIndex(Math.min(Math.max(indicatorIndex, 0), 2));
  }, 150), []);
    ...
    <section className="w-full mt-3 rounded-md bg-white py-6 px-4 sm:px-6 border-b border-gray-200">
      <div className="max-w-full mx-auto">
        ...
        <div className="relative">
          ...
          >
            <div className="flex gap-3 min-w-max">
              {isLoading ? [...Array(8)].map((_, i) => <ProductCardSkeleton key={`s1-${i}`} />) 
                        : firstRowProducts.map(item => <ProductCard key={`f-${item._id}`} item={item} />)}
            </div>

            <div className="flex gap-3 min-w-max">
              {isLoading ? [...Array(8)].map((_, i) => <ProductCardSkeleton key={`s2-${i}`} />) 
                        : secondRowProducts.map(item => <ProductCard key={`s-${item._id}`} item={item} />)}
            </div>
          </div>
        </div>
        ...
        {!isLoading && products.length > 0 && (
          <div className="flex justify-center mt-4">
            <div className="flex gap-1">
              {[0,1,2].map(idx => (
                <div key={idx} className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${currentScrollIndex === idx ? "bg-amber-500 w-6" : "bg-gray-300"}`} />
              ))}
            </div>
          </div>
        )}
