...
const Header = () => {
  ...
  return (
    <header className="w-full sticky top-0 z-50 shadow-lg">
              ...
              <>
                <div className="relative hidden md:flex items-center" ref={userMenuRef}>
                  ...
                  {openUserMenu && (
                    <Suspense
                      fallback={
                        <div className="absolute right-0 top-10 bg-white rounded-xl shadow-xl min-w-[240px] z-50 border border-gray-200 p-2">
                          Loading...
                        </div>
                      }
                    >
      ...
      {/* Cart Overlay */}
      {openCartSection && <DisplayCartItems close={() => setOpenCartSection(false)} />}
    </header>
  );
};
