...
const EditProductAdmin = ({ close, data: productData, fetchProductData }) => {
  ...
  return (
    <section className="fixed inset-0 bg-black/50 popupmodel flex items-center justify-center p-4 z-[200]">
      <div className="bg-white w-full max-w-4xl rounded-2xl overflow-hidden shadow-xl border border-slate-200 min-h-[52vh]">
        {/* Header */}
        <div className="flex justify-between items-center px-6 py-5 border-b border-slate-200 bg-white">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1 max-w-2xl">
              Update product details using a guided wizard. Your changes will reflect across the store.
            </p>
          </div>
          <button
            onClick={close}
        ...
        {/* Wizard Form */}
        <form onSubmit={handelSubmit} className="flex flex-col max-h-[82vh]">
          {/* Step Pills */}
              ...
              {steps.map((label, index) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => setStep(index)}
                    className={`flex items-center gap-3 px-4 py-2 rounded-full text-sm border min-w-fit transition-all whitespace-nowrap
                      ${
                        isActive
          ...
          <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-6 space-y-6">
            ...
            {step === 0 && (
              <div className="grid gap-6 sm:grid-cols-2">
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={handelChange}
                      placeholder="Enter product name"
                      className="w-full h-11 px-3 rounded-lg bg-slate-50 border border-slate-200 text-sm focus:ring-2 focus:ring-amber-400/60 focus:border-amber-500 outline-none"
                      required
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={handelChange}
                      placeholder="Enter product description"
                      className="w-full px-3 py-3 rounded-lg bg-slate-50 border border-slate-200 text-sm focus:ring-2 focus:ring-amber-400/60 focus:border-amber-500 outline-none resize-none"
                      rows="5"
                      required
                    />
                    <p className="text-xs text-slate-400 mt-1">
                      Be descriptive — highlight quality, usage and key features.
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  {/* Quick info */}
                  <div className="p-4 rounded-xl border border-amber-100 bg-amber-50/60 text-sm text-amber-900">
                    <p className="font-medium mb-1.5">Tip</p>
                    <p className="text-sm">
                      Basic info appears in listings and search results. Keep the title short and clear.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-2">
                    Use clear images with a clean background. Multiple angles increase trust.
                  </p>
                </div>
                ...
                <div className="flex flex-wrap gap-4 mt-2">
                  {data.image.map((img, index) => (
                    <div
                      key={index}
                      className="relative h-28 w-28 rounded-lg overflow-hidden shadow-sm border border-slate-200 bg-slate-50"
                    >
                      <img
                        src={img}
                        alt="Uploaded"
                      ...
                      <button
                        ...
                        className="absolute bottom-2 right-2 p-1.5 bg-rose-600 hover:bg-rose-700 text-white rounded-full shadow"
                      >
                        <MdOutlineDeleteOutline size={14} />
                      </button>
                    </div>
              ...
              <div className="grid gap-6 sm:grid-cols-2">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    />
                    <p className="text-xs text-slate-400 mt-1">
                      If discount is percentage, keep value 0–100.
                    </p>
                  </div>

                  <div className="p-4 rounded-xl border border-emerald-100 bg-emerald-50/70 text-sm text-emerald-900">
                    <p className="font-medium mb-1">Suggestion</p>
                    <p className="text-sm">
                      Keep stock & pricing realistic. Low stock can be used for urgency messaging.
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap mt-3 gap-2">
                    {data.category.map((c, index) => (
                      <div
                        key={index}
                        className="bg-amber-50 text-sm px-3 py-1.5 rounded-full flex items-center gap-2 border border-amber-100"
                      >
                ...
                <div>
                  ...
                  <div className="flex flex-wrap mt-3 gap-2">
                    {data.subCategory.map((c, index) => (
                      <div
                        key={index}
                        className="bg-slate-50 text-sm px-3 py-1.5 rounded-full flex items-center gap-2 border border-slate-200"
                      >
                        <span className="text-sm">{c.name}</span>
                        <IoClose
                          className="cursor-pointer text-xs hover:text-slate-900"
            ...
            {step === 4 && (
              <div className="space-y-4">
                <div className="space-y-3">
                  {Object.keys(data.more_details).length === 0 && (
                    <div className="border border-dashed border-slate-200 rounded-lg px-3 py-4 text-sm text-slate-500">
                      No additional fields yet. You can add custom details like Material, Brand, Shelf life, etc.
                    </div>
                  )}
                ...
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  <button
                    type="button"
                    onClick={() => setOpenAddField(true)}
                    className="px-4 py-2 h-11 rounded-lg bg-slate-100 hover:bg-slate-200 text-sm font-semibold text-slate-800"
                  >
                    Add more field
                  </button>
          ...
          {/* Wizard Footer */}
          <div className="px-4 sm:px-6 py-4 border-t border-slate-200 bg-slate-50 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                onClick={handlePrev}
                disabled={step === 0}
                className="px-4 h-11 rounded-lg border border-slate-300 text-sm text-slate-700 bg-white hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Back
              </button>
              ...
              {step < steps.length - 1 ? (
                <button
                  type="button"
                  onClick={handleNext}
                  className="px-5 h-11 rounded-lg bg-amber-500 text-white text-sm font-semibold hover:bg-amber-600"
                >
                  Next
                </button>
              ) : (
                <button
                  type="submit"
                  className="px-5 h-11 rounded-lg bg-emerald-600 text-white text-sm font-semibold hover:bg-emerald-700"
                >
                  Update product
                </button>
              )}
